...
const RAW_ST2_QUESTIONS = (typeof window !== "undefined" && (window.ST2_RAW_QUIZ_QUESTIONS || window.QUIZ_QUESTIONS)) 
  || (typeof ST2_RAW_QUIZ_QUESTIONS !== "undefined" ? ST2_RAW_QUIZ_QUESTIONS : []);

const QUIZ_QUESTIONS = (() => {
  const syllabusMods = (typeof window !== "undefined" && window.SYLLABUS_MODULES)
    || (typeof SYLLABUS_MODULES !== "undefined" ? SYLLABUS_MODULES : []);
  const endtermPool = (typeof window !== "undefined" && window.ENDTERM_100_QUESTIONS)
    || (typeof ENDTERM_100_QUESTIONS !== "undefined" ? ENDTERM_100_QUESTIONS : []);
  
  const qMap = new Map();
  RAW_ST2_QUESTIONS.forEach(q => qMap.set(q.id, q));
  endtermPool.forEach(q => {
    if (!qMap.has(q.id)) {
      qMap.set(q.id, q);
    }
  });

  if (!syllabusMods || syllabusMods.length === 0) return [...RAW_ST2_QUESTIONS];
  ...
  // Append any remainder questions from RAW_ST2_QUESTIONS if not explicitly listed in module q_ids
  RAW_ST2_QUESTIONS.forEach(q => {
    if (!visited.has(q.id)) {
      ordered.push(q);
      visited.add(q.id);
    }
...
function showFinalResultsScreen() {
  ...
  const answered = qList.filter(q => answers[q.id] !== undefined).length;
  ...
  examQs.forEach((q, idx) => {
    ...
    headerDiv.innerHTML = `
      <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
        ...
        <span class="tag-badge tag-starred">⭐ Sample End term Q</span>

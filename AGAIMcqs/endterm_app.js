...
const ALL_ENDTERM_AVAILABLE_QUESTIONS = (typeof window !== "undefined" && window.ENDTERM_QUIZ_QUESTIONS) 
  || (typeof ENDTERM_QUIZ_QUESTIONS !== "undefined" ? ENDTERM_QUIZ_QUESTIONS : []);

const SAMPLE_100_QUESTIONS = (typeof window !== "undefined" && window.ENDTERM_100_QUESTIONS)
  || (typeof ENDTERM_100_QUESTIONS !== "undefined" ? ENDTERM_100_QUESTIONS : []);

const ORIGINAL_QUESTIONS = (typeof window !== "undefined" && window.ENDTERM_ORIGINAL_QUESTIONS)
  || (typeof ENDTERM_ORIGINAL_QUESTIONS !== "undefined" ? ENDTERM_ORIGINAL_QUESTIONS : []);

// Practice questions: strictly ordered by ENDTERM_SYLLABUS_MODULES q_ids
const PRACTICE_QUESTIONS = (() => {
  const syllabusMods = (typeof window !== "undefined" && window.ENDTERM_SYLLABUS_MODULES)
    || (typeof ENDTERM_SYLLABUS_MODULES !== "undefined" ? ENDTERM_SYLLABUS_MODULES : []);
  
  const qMap = new Map();
  ORIGINAL_QUESTIONS.forEach(q => qMap.set(q.id, q));
  ALL_ENDTERM_AVAILABLE_QUESTIONS.forEach(q => qMap.set(q.id, q));
  SAMPLE_100_QUESTIONS.forEach(q => {
...
const QUIZ_QUESTIONS = PRACTICE_QUESTIONS;
...
function getExamQuestions() {
  return QUIZ_QUESTIONS.filter(q => q.starred || q.isStarred || q.tag === "Sample End term Q");
}
...
function calcTotalScore(mode = (isExamMode ? "exam" : "practice")) {
  if (mode === "exam") {
    const examQs = getExamQuestions();
    return examQs.reduce((sum, q) => {
      return sum + (isQuestionAnswerCorrect(q, userExamAnswers[q.id]) ? (q.points || 1) : 0);
    }, 0);
  } else {
    return QUIZ_QUESTIONS.reduce((sum, q) => {
      return sum + (isQuestionAnswerCorrect(q, userPracticeAnswers[q.id]) ? (q.points || 1) : 0);
    }, 0);
  }
}
...
function renderSidebarAccordion() {
  ...
  ENDTERM_SYLLABUS_MODULES.forEach((mod) => {
    ...
    modQuestions.forEach((q) => {
      ...
      // Starred indicator at top-right corner
      if (q.starred || q.isStarred || q.tag === "Sample End term Q") {
        qBtn.classList.add("is-starred-q");
        const starSpan = document.createElement("span");
        starSpan.className = "q-star-corner";
        starSpan.textContent = "⭐";
        starSpan.title = "Sample End Term Question";
        qBtn.appendChild(starSpan);
      }

      qBtn.title = (q.starred || q.isStarred ? "⭐ " : "") + `Q${gIndex + 1}: ${q.topic}`;
...
function renderTopProgressTrack() {
  ...
  if (isExamMode) {
    ...
    const totalMarks = examQs.reduce((sum, q) => sum + (q.points || 1), 0);
    const answered = examQs.filter(q => userExamAnswers[q.id] !== undefined).length;
    ...
    if (lblQ) lblQ.textContent = `📝 Exam Mode (${total} Starred Questions)`;
...
function renderModuleGuideView() {
  const guideMods = (typeof window !== "undefined" && window.ENDTERM_MODULE_GUIDES)
    || (typeof ENDTERM_MODULE_GUIDES !== "undefined" ? ENDTERM_MODULE_GUIDES : {});
  const guide = guideMods[activeGuideModuleId] || guideMods["et_mod1"];
  if (!guide) return;
  ...
  if (simContainer && typeof Visualizers !== "undefined") {
    ...
    } else if (activeGuideModuleId === "et_mod3" && Visualizers.renderAgenticReActSimulator) {
      Visualizers.renderAgenticReActSimulator("guideModSimContainer");
    } else if (activeGuideModuleId === "et_mod4" && Visualizers.renderAgenticWorkflowSimulator) {
...
function renderMainQuestionView() {
  ...
  if (heading) heading.textContent = (q.starred || q.isStarred ? "⭐ " : "") + (q.topic || "");
  ...
  // Primary Question Tag
  const tagBadge = document.getElementById("examPrimaryQuestionTag");
  if (tagBadge) {
    if (q.starred || q.isStarred || q.tag === "Sample End term Q") {
      tagBadge.textContent = "⭐ Sample End term Q";
      tagBadge.className = "exam-tag-top tag-starred";
    } else {
      tagBadge.textContent = "🎯 Sir's Original Revision Question";
      tagBadge.className = "exam-tag-top";
    }
  }
...
function showFinalResultsScreen() {
  ...
  const qList = isExamMode ? getExamQuestions() : QUIZ_QUESTIONS;
  const total = qList.length;
  const answered = Object.keys(answers).length;
  const correct = qList.filter(q => isQuestionAnswerCorrect(q, answers[q.id])).length;
  ...
  let feedback = "";
  if (pct >= 85) {
    feedback = "🌟 Outstanding! You have thoroughly mastered all End Term Deep Learning & Computer Vision topics!";
...
function cleanMathFormulas(text) {
  ...
  // Normalize double backslashes before LaTeX commands and symbols
  s = s.replace(/\\([a-zA-Z,;:!_&|%{}])/g, (m, c) => '\\' + c);
  ...
  s = s.replace(/\\begin\{cases\}([\s\S]*?)\\end\{cases\}/g, (m, body) => {
    ...
    const renderedRows = rows.map(r => {
      const parts = r.split('&').map(p => cleanMathFormulas(p.trim()));
      if (parts.length >= 2) {
        return `<span>${parts[0]}, &nbsp;&nbsp; ${parts[1]}</span>`;
      }
      return `<span>${parts[0]}</span>`;
    });
    return `<span style="display:inline-flex; align-items:center; vertical-align:middle; margin:0 4px;"><span style="font-size:1.8em; font-weight:300; line-height:1; margin-right:5px; opacity:0.85;">{</span><span style="display:inline-flex; flex-direction:column; text-align:left; line-height:1.45; font-size:0.95em;">${renderedRows.join('')}</span></span>`;
  });

  // \begin{bmatrix} ... \end{bmatrix}
  s = s.replace(/\\begin\{bmatrix\}([\s\S]*?)\\end\{bmatrix\}/g, (m, body) => {
    const rows = body.split(/\\\\/).map(r => r.trim()).filter(Boolean);
    const renderedRows = rows.map(r => {
      const cols = r.split('&').map(c => cleanMathFormulas(c.trim()));
      return `<span>${cols.join('&nbsp;&nbsp;&nbsp;')}</span>`;
    });
    return `<span style="display:inline-flex; align-items:center; vertical-align:middle; margin:0 4px; border-left:2px solid currentColor; border-right:2px solid currentColor; border-radius:3px; padding:2px 7px;"><span style="display:inline-flex; flex-direction:column; text-align:center; line-height:1.35; font-size:0.92em;">${renderedRows.join('')}</span></span>`;
  });

  // \xrightarrow{label}
  let xarrIdx;
  while ((xarrIdx = s.indexOf('\\xrightarrow{')) !== -1) {
    const g = extractBraceGroup(s, xarrIdx + 12);
    if (!g) break;
    const cleanLabel = cleanMathFormulas(g.content).trim();
    s = s.slice(0, xarrIdx) + ` ──[${cleanLabel}]──► ` + s.slice(g.endIndex);
  }
  s = s.replace(/\\xrightarrow\b/g, ' ──► ');

  // \texttt{...}
  let ttIdx;
  while ((ttIdx = s.indexOf('\\texttt{')) !== -1) {
    const g = extractBraceGroup(s, ttIdx + 7);
    if (!g) break;
    const code = g.content.trim();
    s = s.slice(0, ttIdx) + `<code style="font-family:'DM Mono',monospace; background:rgba(99,102,241,0.12); color:#0ea5e9; padding:2px 6px; border-radius:4px; font-size:0.9em; font-weight:600;">${code}</code>` + s.slice(g.endIndex);
  }
  s = s.replace(/\\texttt\b/g, '');

  // Concatenation & norm operators
  s = s.replace(/\\mathbin\{\\Vert\}|\\mathbin\{\|\}/g, ' ‖ ');
  ...
  // Escaped underscores and percent signs
  s = s.replace(/\\_/g, '_');
  s = s.replace(/\\%/g, '%');
  ...
  s = s.replace(/\\hat\{y\}|\by_hat\b/g, 'ŷ');
  s = s.replace(/\\hat\{w\}|\bw_hat\b/g, 'ŵ');
  s = s.replace(/\\hat\{x\}|\bx_hat\b/g, 'x̂');
  ...
  // Math operators like \arg\max, \argmax, \min, \max, etc.
  s = s.replace(/\\arg\s*\\max(?=[^a-zA-Z]|$)|\\argmax(?=[^a-zA-Z]|$)/g, 'argmax');
  s = s.replace(/\\arg\s*\\min(?=[^a-zA-Z]|$)|\\argmin(?=[^a-zA-Z]|$)/g, 'argmin');
  s = s.replace(/\\max(?=[^a-zA-Z]|$)/g, 'max');
  s = s.replace(/\\min(?=[^a-zA-Z]|$)/g, 'min');

  // Trig / transcendental functions
  s = s.replace(/\\cos(?=[^a-zA-Z]|$)/g, 'cos');
  s = s.replace(/\\sin(?=[^a-zA-Z]|$)/g, 'sin');
  s = s.replace(/\\tan(?=[^a-zA-Z]|$)/g, 'tan');
  s = s.replace(/\\tanh(?=[^a-zA-Z]|$)/g, 'tanh');
  ...
  s = s.replace(/\\mathcal\{F\}/g, 'F');
  s = s.replace(/\\mathcal\{D\}/g, 'D');
  s = s.replace(/\\mathcal\{M\}/g, 'M');
  s = s.replace(/\\mathcal\{T\}/g, 'T');
  s = s.replace(/\\mathcal\{H\}/g, 'H');
  s = s.replace(/\\mathcal\{O\}/g, 'O');
  s = s.replace(/\\mathcal\{C\}/g, 'C');
  ...
  s = s.replace(/\\theta(?=[^a-zA-Z]|$)|\\vartheta(?=[^a-zA-Z]|$)/g, 'θ');
  ...
  // Single token superscripts e.g. W^O, K^T, x^2, \Sigma_{t=1}^T
  ...
  examQs.forEach((q, idx) => {
    ...
    headerDiv.innerHTML = `
      <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
        <span class="full-exam-q-num">Question ${idx + 1} of ${examQs.length}</span>
        ${(q.starred || q.isStarred || q.tag === "Sample End term Q") ? '<span class="tag-badge tag-starred">⭐ Sample End term Q</span>' : '<span class="tag-badge">🎯 Sir\'s Original</span>'}
      ...
      <span class="full-exam-topic">${(q.starred || q.isStarred) ? '⭐ ' : ''}${escapeHtml(q.topic || "")}</span>
    `;
...
// Global Robust Initialization with Re-entrancy Guard

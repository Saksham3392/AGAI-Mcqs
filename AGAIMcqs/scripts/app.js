...
const STORAGE_KEY = "AGAI_ST2_FINAL_STATE_V7";
let currentQIndex = 0;
...
function loadStoredState() {
  try {
    ...
    if (raw) {
      ...
      userExamAnswers = (data.userExamAnswers && typeof data.userExamAnswers === "object") ? data.userExamAnswers : {};
...
function clearCurrentQuestionChoice() {
  ...
  if (q && userExamAnswers[q.id] !== undefined) {
...
function calcTotalScore() {
  return QUIZ_QUESTIONS.reduce((sum, q) => {
    return sum + (userExamAnswers[q.id] === q.correct ? (q.points || 1) : 0);
  }, 0);
}
  ...
  SYLLABUS_MODULES.forEach((mod) => {
    ...
    modQuestions.forEach((q) => {
      ...
      if (userExamAnswers[q.id] !== undefined) {
        if (userExamAnswers[q.id] === q.correct) {
